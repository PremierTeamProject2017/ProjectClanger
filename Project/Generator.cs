...
namespace Project
{
    class Generator
    {
        ...
        public String genUserName()
        {
            ...
            string LastUserID = dr[0].ToString();
            int iUserID = Int32.Parse(LastUserID);
            iUserID += 1;
            string sUserID = iUserID.ToString();
            string newUserID = "PI" + sUserID;
            //MessageBox.Show(newUserID);
            return newUserID;
        }

        //Code taken from kratos_Vimal(https://forums.asp.net/t/1685777.aspx?Generate+Random+password+using+C+) and amended to suit our needs
